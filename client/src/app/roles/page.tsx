...
export default function AssignRoles() {
  ...
  const loadBlockchainData = async () => {
    try {
      ...
      const rmsCount = await contract.methods.supplierCtr().call()
      const manCount = await contract.methods.producerCtr().call()
      const disCount = await contract.methods.distributorCtr().call()
      const retCount = await contract.methods.sellerCtr().call()

      const rms = await Promise.all(
        Array(parseInt(rmsCount))
          .fill(null)
          .map((_, i) => contract.methods.SUPPLIERS(i + 1).call())
      )
      const man = await Promise.all(
        Array(parseInt(manCount))
          .fill(null)
          .map((_, i) => contract.methods.PRODUCERS(i + 1).call())
      )
      ...
      const ret = await Promise.all(
        Array(parseInt(retCount))
          .fill(null)
          .map((_, i) => contract.methods.SELLERS(i + 1).call())
      )
