...
export default function Supply() {
  ...
  const handlerSubmitRMSsupply = async (event: React.FormEvent) => {
    ...
    try {
      const account = await getActiveAccount()
      setCurrentAccount(account)
      const receipt = await supplyChain.methods.supplyProduct(rmsId).send({ from: account })
      if (receipt) {
        loadBlockchainData()
        setRmsId('')
  ...
  const handlerSubmitManufacturing = async (event: React.FormEvent) => {
    ...
    try {
      const account = await getActiveAccount()
      setCurrentAccount(account)
      const receipt = await supplyChain.methods.processProduct(manId).send({ from: account })
      if (receipt) {
        loadBlockchainData()
        setManId('')
  ...
  const handlerSubmitDistribute = async (event: React.FormEvent) => {
    ...
    try {
      const account = await getActiveAccount()
      setCurrentAccount(account)
      const receipt = await supplyChain.methods.distributeProduct(disId).send({ from: account })
      if (receipt) {
        loadBlockchainData()
        setDisId('')
  ...
  const handlerSubmitRetail = async (event: React.FormEvent) => {
    ...
    try {
      const account = await getActiveAccount()
      setCurrentAccount(account)
      const receipt = await supplyChain.methods.listForSale(retId).send({ from: account })
      if (receipt) {
        loadBlockchainData()
        setRetId('')
  ...
  const handlerSubmitSold = async (event: React.FormEvent) => {
    ...
    try {
      const account = await getActiveAccount()
      setCurrentAccount(account)
      const receipt = await supplyChain.methods.markProductSold(soldId).send({ from: account })
      if (receipt) {
        loadBlockchainData()
        setSoldId('')
