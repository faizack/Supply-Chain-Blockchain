...
export default function AddMed() {
  ...
  const loadBlockchainData = async () => {
    try {
      ...
      const medCtr = Number(await contract.methods.productCtr().call())
  ...
  const handlerChangeDesMED = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const handlerSubmitMED = async (event: React.FormEvent) => {
    ...
    try {
      const account = await getActiveAccount()
      setCurrentAccount(account)
      const receipt = await supplyChain.methods.addProduct(medName, medDes).send({ from: account })
      if (receipt) {
        loadBlockchainData()
        setMedName('')
        setMedDes('')
